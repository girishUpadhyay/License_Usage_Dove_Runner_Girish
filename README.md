...
- **Data layer**: `src/app/api/licenses/route.ts` (GET) and `src/app/api/licenses/[id]/route.ts` (PATCH) are real Next.js Route Handlers backed by an in-memory store (`src/features/licenses/data/`), not a statically-imported JSON file — the UI fetches over HTTP like it would against a real backend. `src/features/licenses/hooks/useLicenses.ts` wraps the fetch in SWR for caching/revalidation.
- **Table**: search (debounced), status/plan filters, sortable columns, and pagination are all client-side state, pulled into a `useLicensesPage` hook (`src/features/licenses/hooks/useLicensesPage.ts`) that `LicensesPageClient.tsx` just wires to JSX. The table itself (`src/features/licenses/components/LicenseTable.tsx`) is a plain MUI `Table` — at ~80 mock records, plain DOM rows render instantly, so there's no virtualization to add complexity for a dataset this size.
- **Detail view + edit**: clicking a row opens an MUI `Drawer` (`LicenseDetailDrawer.tsx`) with the full record and an "edit seats allowed" form that validates client-side and server-side (shared validation logic in `src/features/licenses/utils/license-validation.ts`) before updating in-memory state — no persistence beyond the running process, per the brief.
